...
The demo requires additional data to be downloaded from a Zenodo repository. On Unix or MacOS-type systems, run the `download_data.sh` script to automatically download the needed files:

    chmod u+x download_data.sh
    ./download_data.sh

For Windows computers, there is an equivalent PowerShell script. **Beware!** This might not work on all Windows machines due to restrictions on the execution of PowerShell scripts without administrator privileges.
